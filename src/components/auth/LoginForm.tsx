  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9] flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8 relative font-sans">
      ...
      <div className="fixed top-6 left-6 z-20">
        <button
          onClick={() => router.push("/")}
          className="group flex items-center gap-3 px-4 py-2.5 bg-white border border-slate-200 hover:border-brand-gold text-slate-600 hover:text-slate-900 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer"
        >
        ...
        <div className="bg-white pt-10 pb-10 px-8 sm:px-10 shadow-[0_15px_40px_-10px_rgba(0,0,0,0.04)] rounded-3xl border border-slate-200/80 border-t-4 border-t-brand-gold w-full relative">
          <div className="space-y-6">
            ...
            <form action={formAction} className="space-y-6">
              {state?.error && (
                <div className="flex items-center gap-2.5 p-3.5 bg-red-50 border border-red-200/50 text-red-700 rounded-xl text-xs font-semibold animate-in fade-in duration-200">
              ...
              <div className="space-y-4">
                <div className="space-y-1.5 group/input">
                  <label className="text-[10px] font-bold uppercase text-slate-400 ml-1 tracking-[0.2em] block group-focus-within/input:text-slate-800 transition-colors">
                    Correo Electrónico
                  </label>
                  <div className="relative rounded-xl shadow-sm">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400 group-focus-within/input:text-slate-800 transition-colors">
                      <Mail size={15} />
                    </div>
                    <input
                      name="email"
                      type="email"
                      className="w-full py-3 pl-10 pr-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:bg-white focus:border-brand-gold/60 focus:ring-4 focus:ring-brand-gold/5 outline-none transition-all placeholder:text-slate-400"
                ...
                <div className="space-y-1.5 group/input">
                  ...
                  <div className="relative rounded-xl shadow-sm">
                    ...
                    <input
                      ...
                      className="w-full py-3 pl-10 pr-10 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:bg-white focus:border-brand-gold/60 focus:ring-4 focus:ring-brand-gold/5 outline-none transition-all placeholder:text-slate-400"
                      placeholder="••••••••"
                      required
                    />
                    <button
